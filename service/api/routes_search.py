...
⚠️ **대체 창구(2026-09-09)**: 새 화면은 ``GET /file-search``(``routes_file_search``)를 쓴다 — 같은 단어 절·같은
점수식으로 순서를 매기되 개수·칩·페이징·정렬을 주고 12배 빠르다. 이 라우트는 기존 멀티모달 검색 화면(종류별 그룹
응답)과 과제 산출물 근거로 **남긴다**(설계이력 2026-09-09 (6)~(8)).
"""
